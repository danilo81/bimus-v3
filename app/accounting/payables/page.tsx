...
export default function PayablesPage() {
    ...
    useEffect(() => {
        getGlobalPayables().then(res => {
            setData(res);
            setLoading(false);
        });
    }, []);

    const filteredItems = data?.items?.filter((pay: any) =>
        pay.supplier.toLowerCase().includes(searchTerm.toLowerCase()) ||
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold font-headline flex items-center gap-3">
                            <CreditCard className="h-8 w-8 text-red-500" /> Cuentas por Pagar
                        </h1>
                ...
                <Card className="bg-card border-amber-500/20 shadow-xl relative overflow-hidden">
                    ...
                    <CardContent>
                        ...
                        <p className="text-[9px] font-black text-amber-500 uppercase mt-2 flex items-center gap-1">
                            <AlertTriangle className="h-3 w-3" /> {data?.overdueCount || 0} Facturas pendientes
                        </p>
                    </CardContent>
                </Card>
                            ...
                            <TableBody>
                                {filteredItems.map((pay: any) => (
                                    <TableRow key={pay.id} className="border-white/5 hover:bg-white/3 transition-colors group">
                                        ...
                                        <TableCell className="text-right font-mono font-black text-red-500 text-sm">
                                        ...
                                        <TableCell className="text-center">
                                            <Badge variant="outline" className={cn("text-[8px] font-black uppercase border-white/10 tracking-widest",
                                                ...
                                                {pay.status === 'paid' ? 'Pagado' : pay.status === 'overdue' ? 'Vencido' : 'Pendiente'}
                                            </Badge>
                                        </TableCell>
