...
function IfcModelRenderer({ model, elements, onSelect, activeTool }: { model: any, elements: BimElement[], onSelect: (id: string, multi: boolean) => void, activeTool: string | null | undefined }) {
  ...
  useEffect(() => {
    // Hide the original model so it doesn't render
    model.visible = false;

    const modelElements = elements.filter(el => el.geometry.type === 'ifc' && el.geometry.args.modelID === model.modelID);
...
export function BimViewer({
  ...
  return (
    ...
    >
      <Canvas
        shadows={modelConfig.showShadows}
