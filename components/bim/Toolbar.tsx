...
import { MousePointer2, Move, RotateCw, Maximize, Box, Cylinder, Square, Trash2, Settings, DoorOpen, AppWindow, Minus, Circle, Magnet, AlignLeft, AlignCenter, AlignRight, Group, Ungroup, Edit, Eye, EyeOff, LogOut, LassoSelect, BoxSelect, Wand2, Ruler, Type, Tag, Spline, CircleDashed, ArrowLeftRight, Upload, Scissors, Cloud } from 'lucide-react';
          ...
          {toolGroup === 'bim' && (
            <div className="flex items-center gap-1 border-r border-zinc-700 pr-2 mr-1">
              ...
              <ToolButton
                ...
                label="IFC Cloud"
                onClick={onOpenIfcCloud}
              />
              <div className="w-px h-6 bg-zinc-700 mx-1"></div>
              <ToolButton
                icon={<Tag className="w-4 h-4 text-emerald-400" />}
                label="Asignar Partidas (5D)"
                active={activeTool === 'assign'}
                onClick={() => handleToolClick('assign')}
              />
            </div>
          )}
