...
function BimMesh({
  ...
  lineTypes?: LineType[];
  isMapped?: boolean;
  isMappedToActiveTarget?: boolean;
}) {
  ...
  if (isSelected) {
    ...
    color = highlightColor.getStyle();
  } else if (isMappedToActiveTarget) {
    color = '#10b981'; // Dynamic Emerald Green
  } else if (isMapped) {
    color = '#059669'; // Subtle Mapped Green
  }
    ...
    return (
      ...
      >
        {wallType.layers.map((layer, index) => {
          ...
          let finalColor = layerColor.getStyle();
          if (isSelected) finalColor = '#3b82f6';
          else if (hovered) finalColor = highlightColor.getStyle();
          else if (isMappedToActiveTarget) finalColor = '#10b981';
          else if (isMapped) finalColor = '#059669';
  ...
  return (
    <group>
      {elements.map((element) => {
        ...
        const isMapped = mappedElements.some(m => m.elementId === element.id);
        const isMappedToActiveTarget = !!(activeAssignmentTarget && mappedElements.some(m => m.elementId === element.id && m.projectItemId === activeAssignmentTarget));
        ...
        return (
          <BimMesh
            ...
            isMapped={isMapped}
            isMappedToActiveTarget={isMappedToActiveTarget}
